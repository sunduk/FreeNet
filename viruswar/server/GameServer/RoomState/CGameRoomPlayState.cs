...
namespace GameServer.RoomState
{
    ...
    class CGameRoomPlayState : IState
    {
        ...
        {
            this.room = room;
            this.room.state_manager.register_message_handler(this, PROTOCOL.MOVING_REQ, moving_req);
            this.room.state_manager.register_message_handler(this, PROTOCOL.TURN_FINISHED_REQ, turn_finished);
        ...
        void battle_start()
        {
            ...
            room.each_player(player =>
            {
                ...
                // 플레이어들의 세균 위치 전송.
                msg.Push((byte)room.get_player_count());
                room.each_player(_player =>
                {
                    ...
                    msg.Push(cell_count);
                    // 플레이어의 세균들의 위치정보.
                    _player.viruses.ForEach(position => msg.PushInt16(position));
                });

                // 첫 턴을 진행할 플레이어 인덱스.
                msg.Push(room.get_current_player().player_index);
        ...
        void start_turn()
        {
            Packet msg = Packet.Create((short)PROTOCOL.START_PLAYER_TURN);
            msg.Push(room.get_current_player().player_index);
            room.broadcast(msg);
        }
        ...
        {
            // 플레이어 데이터 초기화.
            room.each_player(player => player.reset());
        ...
        void put_virus(byte player_index, short position)
        {
            ...
            room.get_player(player_index).add_cell(position);
        }
        ...
        void remove_virus(byte player_index, short position)
        {
            ...
            room.get_player(player_index).remove_cell(position);
        }
        ...
        public void moving_req(CPlayer sender, Packet received_data)
        {
            room.clear_received_protocol();

            short begin_pos = received_data.PopInt16();
            short target_pos = received_data.PopInt16();

            // sender차례인지 체크.
            if (!room.is_current_player(sender))
            {
                room.error(sender);
                return;
            }
            ...
            CPlayer opponent = room.get_opponent_player();
            infect(target_pos, sender, opponent);

            // 최종 결과를 broadcast한다.
            Packet msg = Packet.Create((short)PROTOCOL.PLAYER_MOVED);
        ...
        public void turn_finished(CPlayer sender, Packet msg)
        {
        ...
        void turn_end()
        {
            ...
            if (!CHelper.can_play_more(table_board, room.get_opponent_player(), room.get_players()))
            {
                game_over();
                return;
            }
        ...
        void game_over()
        {
            ...
            int count_1p = room.get_player(0).get_virus_count();
            int count_2p = room.get_player(1).get_virus_count();
            ...
            else
            {
                if (count_1p > count_2p)
                {
                    win_player_index = room.get_player(0).player_index;
                }
                else
                {
                    win_player_index = room.get_player(1).player_index;
                }
            }


            Packet msg = Packet.Create((short)PROTOCOL.GAME_OVER);
